    ...
    use_effect_with(
        ...
        move |(textarea_ref, highlight_ref, line_numbers_ref)| {
            if let (Some(textarea_el), Some(highlight_el), Some(line_numbers_el)) = (
                ...
                let textarea_for_scroll = textarea_el.clone();
                let highlight_for_scroll = highlight_el.clone();
                let line_numbers_for_scroll = line_numbers_el.clone();
...
fn compile_source(source: &str, unsafe_fast: bool) -> String {
    ...
    match compiler.compile_with_options(&options) {
        ...
        Err(e) => format!("<span class=\"error\">Compilation Error:\n{}</span>", escape_html(&e.to_string())),
    }
}
